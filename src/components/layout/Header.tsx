...
import { 
  Moon, Sun, Search, Bell, Zap, ChevronRight, ChevronsLeft,
  Home, ArrowRight
} from 'lucide-react';
import { cn } from '../../lib/utils';
import { useBreadcrumbs } from '../../routes/utils';
...
export function Header({ collapsed = false, onToggleSidebar, className }: HeaderProps) {
  ...
  // Get breadcrumbs from route metadata
  ...
  return (
    <motion.header
      ...
      className={cn(
        'fixed top-0 right-0 h-16 z-50 flex flex-col transition-all duration-300',
        className
      )}
    >
      {/* Top Bar: Logo + Search + Actions */}
      <div className="flex items-center justify-between px-4 sm:px-6 py-2 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-700/50">
